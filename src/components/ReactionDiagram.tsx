...
const ReactionDiagram = () => {
    ...
    if (load.type === 'point') {
      return (
        ...
        >
          ...
          <span className="text-xs mt-1 text-gray-600">{load.force}N</span>
        </div>
      );
    }
    ...
    return (
      ...
      >
        <div className="flex flex-col items-center">
          ...
          <span className="text-xs mt-1 text-gray-600">{load.force}N/m</span>
        </div>
      </div>
    );
  };
  ...
  const renderReaction = (force: number, position: number, label: string) => {
    ...
    return (
      ...
      >
        ...
        <span className="text-xs mt-1 text-gray-600">{Math.abs(force).toFixed(1)}N</span>
  ...
  return (
    <div className="relative h-[200px] bg-gray-50 rounded-lg p-4">
      {/* Beam */}
      <div className="absolute top-1/2 left-0 right-0 h-2 bg-gray-400 rounded" />
      ...
      <div className="absolute bottom-4 left-0 right-0">
        {markers.map((marker, i) => (
          ...
          >
            ...
            <span className="text-sm text-gray-600 mt-1">{marker.label}mm</span>
          </div>
        ))}
      </div>
    </div>
